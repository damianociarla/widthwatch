...
Regional SNS topics receive App Runner 5xx, CPU, memory, completed/failed/rejected scan, transfer-limit, CloudFront 5xx/request-surge and WAF block signals. An hourly external GitHub canary reads the expected control-plane state, verifies health in both states, checks edge blocking when disabled, and crosses scan, polling, report delivery and robot-policy seams when enabled.
