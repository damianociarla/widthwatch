...
Every failed hosted job becomes one sanitized job outcome. HTTP admission exposes only a stable failure code and generic message; the operational adapter writes one JSON line with job ID, phase, duration and optional numeric transfer metadata. Raw errors, target URLs, hostnames, query strings, page content and stacks never cross this seam. CloudWatch metric filters count total and transfer-limit failures from the stable event, while adapter failures are isolated from job state and queue progress. Bounded egress limits are validated and frozen when the hosted runner is constructed, so an invalid environment prevents startup rather than producing a healthy process that fails every scan.
...
The incident-control plane is versioned with the request path. Regional SNS topics receive App Runner 5xx, CPU, memory, failed-scan, transfer-limit, CloudFront 5xx/request-surge and WAF block alarms. The protected GitHub environment owns `WIDTHWATCH_PUBLIC_SCANNER_ENABLED`; both release and manual deployment workflows pass it to the highest-priority WAF scanner switch. A disabled value blocks exact `POST /v1/scans` requests but preserves health, status and report reads, and it survives ordinary releases until explicitly restored through the runbook.
