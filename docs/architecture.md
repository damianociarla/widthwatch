...
A protected `v*` tag validates its commit ancestry and requires the tag version to match every package, lockfile and public metadata surface. The release then installs and verifies the versioned scanner control-plane revision before deploying the bounded API; npm publication can proceed in parallel after validation. The idempotent GitHub Release waits for the infrastructure migration, application deploy and npm publication. Pull requests and both Node CI jobs protect `main`; merges deploy GitHub Pages. Deployment environments admit only their intended branch or tag patterns, third-party Actions are allowlisted and every Action is pinned to a full SHA. AWS access uses GitHub OIDC and short-lived credentials; production mutation and canary observation use separate roles and environments.
