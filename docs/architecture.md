...
The scanner control plane is independent of the release path. A dedicated fail-closed CloudFront-scope WAF rule group is referenced at priority zero by the ordinary Web ACL, while only `scanner-switch.yml` can update its switch-only stack through narrow GitHub OIDC and CloudFormation execution roles. Release and manual application deploys may bootstrap the stack once in the disabled state and subsequently read only its ARN; they never pass or mutate scanner state. Disable blocks exact `POST /v1/scans` while preserving health, status and report reads. Enable is explicit and fails until both regional SNS email subscriptions are confirmed. The workflow correlates each incident by a caller-supplied change ID and verifies edge behavior after WAF propagation.
...
A protected `v*` tag validates the exact artifact, then deploys the bounded API and publishes npm through trusted OIDC in parallel. The idempotent GitHub Release waits for both. Pull requests and both Node CI jobs protect `main`; merges deploy GitHub Pages. Deployment environments admit only their intended branch or tag patterns, third-party Actions are allowlisted and every Action is pinned to a full SHA. AWS access uses GitHub OIDC and short-lived credentials; production mutation and canary observation use separate roles and environments.
