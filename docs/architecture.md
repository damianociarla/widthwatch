...
The scanner control plane is independent of the application deploy path. A dedicated fail-closed CloudFront-scope WAF rule group is referenced at priority zero by the ordinary Web ACL. The operational `scanner-switch.yml` workflow changes only `PublicScannerEnabled` against the previously installed template. A separate release-called upgrade workflow owns full-template installation: it validates the immutable tag and version contract, reads and preserves the current state, installs the versioned template through the narrow CloudFormation role, records its SHA-256 control-plane revision and verifies the edge contract. Disable blocks exact `POST /v1/scans` while preserving health, status and report reads. Enable is explicit and fails until both regional SNS email subscriptions are confirmed. Both mutation paths share one concurrency group, so template upgrades and incident toggles cannot race.
