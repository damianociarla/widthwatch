...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:#0a0c09;color:var(--ink);font:14px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}button,input{font:inherit}
    ...
    .brand{font-weight:800;letter-spacing:-.04em;font-size:18px}.brand i{color:var(--acid);font-style:normal}.status{color:var(--muted)}.status b{color:var(--acid)}
    ...
    .tick{position:absolute;top:26px;width:2px;height:34px;background:var(--acid);border:0;padding:0;cursor:pointer}.tick.error{background:var(--error);height:46px;top:20px}.tick.warning{background:var(--warn)}.tick span{position:absolute;top:42px;left:50%;transform:translateX(-50%);color:var(--muted);font-size:11px;white-space:nowrap}
    ...
    .empty{color:var(--muted);padding:24px 0}.legend{display:flex;gap:14px;color:var(--muted);font-size:11px;margin-top:10px}
...
<body>
<header><div class="brand">width<i>watch</i></div><div class="status"><b>●</b> scan complete</div></header>
<main><section class="workspace"><div class="meta"><span>range <strong>${report.range.min}—${report.range.max}px</strong></span><span>samples <strong>${report.frames.length}</strong></span><span>duration <strong>${(report.durationMs / 1000).toFixed(1)}s</strong></span></div><div class="timeline" id="timeline"><div class="track"></div></div><div class="viewer"><div class="shot-wrap" id="shotWrap"><img class="shot" id="shot" alt="Responsive capture"><div id="overlays"></div></div></div><div class="legend"><span>● sampled width</span><span>● issue detected</span></div></section>
<aside class="inspector"><h1>${escapeHtml(report.title || "Untitled page")}</h1><div class="url">${escapeHtml(report.url)}</div><div class="summary"><div><strong>${report.summary.errors}</strong><span>errors</span></div><div><strong>${report.summary.warnings}</strong><span>warnings</span></div><div><strong>${comparison?.diffs.length ?? report.summary.sampledWidths}</strong><span>${comparison ? "diffs" : "frames"}</span></div></div><div class="issues" id="issues"></div></aside></main>
<script>const DATA=${payload};
const report=DATA.report, timeline=document.querySelector('#timeline'), shot=document.querySelector('#shot'), shotWrap=document.querySelector('#shotWrap'), issues=document.querySelector('#issues'), overlays=document.querySelector('#overlays');let selected=0;
function severity(frame){return frame.issues.some(i=>i.severity==='error')?'error':frame.issues.some(i=>i.severity==='warning')?'warning':''}
function render(index){selected=index;const frame=report.frames[index];shot.src=frame.screenshot;shot.alt='Capture at '+frame.width+' pixels';shotWrap.style.width=Math.min(frame.width,shotWrap.parentElement.clientWidth-24)+'px';document.querySelectorAll('.tick').forEach((el,i)=>el.style.opacity=i===index?'1':'.45');issues.innerHTML='';overlays.innerHTML='';if(!frame.issues.length){issues.innerHTML='<div class="empty">No deterministic issues at this width.</div>'}
...
report.frames.forEach((frame,index)=>{const tick=document.createElement('button');tick.className='tick '+severity(frame);tick.style.left=((frame.width-report.range.min)/(report.range.max-report.range.min)*100)+'%';tick.setAttribute('aria-label','Show '+frame.width+' pixel capture');tick.innerHTML='<span>'+frame.width+'</span>';tick.onclick=()=>render(index);timeline.append(tick)});render(0);addEventListener('resize',()=>render(selected));</script></body></html>`;
}
