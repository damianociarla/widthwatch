...
`visual` mode is the default. Adaptive scans first run a fast geometry discovery pass, then open a fresh page and capture full-page evidence only at the final schedule. `maxSamples` bounds discovery (24 by default) and `maxCaptureSamples` bounds expensive evidence (8 by default). Error and warning widths, strong transition sides, range endpoints and width coverage drive the deterministic selection. Exact-width scans always capture every requested width, so baseline comparisons remain complete.
...
New reports expose optional `sampling` metadata with the strategy, discovery widths and captured widths. The field is optional on schema-v1 objects so existing report producers remain source-compatible.
