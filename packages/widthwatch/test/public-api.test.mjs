...
test("reporter canonicalizes capture-only fallback findings", () => {
  const finding = { id: "lazy-capture", kind: "clipped-text", severity: "error", width: 640, message: "capture only", elements: [{ selector: "#lazy", tagName: "p", rect: { x: 0, y: 0, width: 10, height: 10 } }] };
  const probe = { width: 640, height: 800, document: { width: 640, height: 800 }, layoutSignature: "x", issues: [], durationMs: 1 };
  const report = { version: 1, url: "https://example.com/", title: "Example", scannedAt: new Date(0).toISOString(), durationMs: 1, range: { min: 640, max: 640, height: 800 }, environment: { browser: "test", platform: "test", packageVersion: "0.4.0" }, probes: [probe], frames: [{ ...probe, issues: [finding], screenshot: "data:image/png;base64," }], transitions: [], summary: { errors: 1, warnings: 0, info: 0, sampledWidths: 1 } };
  const html = generateHtmlReport(report);
...
test("reporter timeline uses one interactive hit layer with clustered passive markers", () => {
  ...
  const report = { version: 1, url: "https://example.com/", title: "Example", scannedAt: new Date(0).toISOString(), durationMs: 1, range: { min: 900, max: 920, height: 800 }, environment: { browser: "test", platform: "test", packageVersion: "0.4.0" }, probes: [probe(900), probe(905), probe(910), probe(920)], frames: [{ ...probe(900), screenshot: "data:image/png;base64," }, { ...probe(920), screenshot: "data:image/png;base64," }], transitions: [], summary: { errors: 0, warnings: 0, info: 0, sampledWidths: 4 } };
  const html = generateHtmlReport(report);
