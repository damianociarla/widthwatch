...
test("scanAtWidths captures exactly the requested deterministic schedule", async (context) => {
  ...
  assert.deepEqual(
    result.frames.map((frame) => frame.width),
    [321, 777],
  );
...
test("adaptive visual scans discover broadly and capture a bounded final schedule", async (context) => {
  ...
  const result = await scanResponsive(url, {
    ...
    pageReady: async (page, { phase }) => {
      ...
      if (phase === "discovery")
        ...
        captureTaint.push(
          await page.evaluate(() => ({ dom: document.body.dataset.discoveryTaint ?? null, storage: localStorage.getItem("widthwatch-discovery-taint") })),
        );
    },
  ...
  assert.deepEqual(
    result.sampling.capturedWidths,
    result.frames.map((frame) => frame.width),
  );
...
test("maxCaptureSamples remains a hard cap when every discovery probe has a finding", async (context) => {
  ...
  assert.deepEqual(
    result.issueRanges.map((range) => [range.from, range.to, range.occurrences]),
    [[320, 1120, 6]],
  );
});
...
test("evidence selection covers distinct finding identities before transition-only widths", async (context) => {
  ...
  assert.deepEqual(
    result.frames.map((frame) => frame.width),
    [320, 480, 640, 800, 1120],
  );
  assert.deepEqual(
    new Set(result.issues.filter((issue) => issue.evidence === "capture").map((issue) => issue.elements[0]?.selector)),
...
test("scanAtReportSchedule reproduces probe and evidence schedules", async (context) => {
  const url = await fixture(context, (_request, response) => html(response, "<main>stable</main>"));
  const baseline = await scanResponsive(url, {
    mode: "visual",
    minWidth: 320,
    maxWidth: 1120,
    initialStep: 160,
  ...
  assert.deepEqual(
    candidate.probes.map((probe) => probe.width),
    baseline.probes.map((probe) => probe.width),
  );
  assert.deepEqual(
    candidate.frames.map((frame) => frame.width),
...
test("pageReady waits for application-specific asynchronous state", async (context) => {
  ...
  const result = await scanAtWidths(url, [320], {
    mode: "layout",
    settleMs: 0,
    pageReady: async (page, hookContext) => {
      calls.push([hookContext.width, hookContext.phase]);
...
test("resources blocked by policy do not consume the request budget", async (context) => {
  ...
  const report = await scanAtWidths(url, [320], {
    mode: "layout",
    viewportHeight: 480,
    settleMs: 0,
    maxRequestsPerNavigation: 1,
    maxTotalRequests: 1,
...
test("maxElements is applied after invisible nodes are filtered", async (context) => {
  ...
  const url = await fixture(context, (_request, response) =>
    html(response, `${hidden}<div id="important" style="width:30px;white-space:nowrap;overflow:hidden">important clipped content</div>`),
  );
...
test("adaptive refinement spends budget across separate transition bands", async (context) => {
  ...
  assert.ok(
    result.frames.some((frame) => frame.width > 720 && frame.width < 1120),
    `Expected refinement near the second transition, got ${result.frames.map((frame) => frame.width).join(", ")}`,
  );
